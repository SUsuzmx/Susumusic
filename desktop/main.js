...
const APP_NAME = 'Susumusic';
const APP_USER_MODEL_ID = 'com.susumusic.desktop';
const APP_ICON_ICO = path.join(__dirname, '..', 'build', 'icon.ico');
const NETEASE_LOGIN_PARTITION = 'persist:susumusic-netease-login';
const NETEASE_LOGIN_URL = 'https://music.163.com/#/login';
const QQ_LOGIN_PARTITION = 'persist:susumusic-qqmusic-login';
...
function sendGlobalHotkeyAction(action) {
  ...
  mainWindow.webContents.send('susumusic-global-hotkey', { action });
}

function unregisterSusumusicGlobalHotkeys() {
...
function configureSusumusicGlobalHotkeys(bindings = []) {
  unregisterSusumusicGlobalHotkeys();
  const results = [];
  const seen = new Set();
...
function shouldEnsureDesktopShortcut() {
  ...
  if (process.env.SUSUMUSIC_NO_DESKTOP_SHORTCUT === '1') return false;
  return app.isPackaged || process.env.SUSUMUSIC_CREATE_DESKTOP_SHORTCUT === '1';
}
...
function ensureDesktopShortcut() {
  ...
  try {
    ...
    const shortcut = {
      ...
      args: '',
      description: 'Susumusic desktop music player',
...
public class SusumusicMousePoll {
  [DllImport("user32.dll")] public static extern short GetAsyncKeyState(int vKey);
}
"@
$prev = $false
while ($true) {
  $down = (([SusumusicMousePoll]::GetAsyncKeyState(4) -band 0x8000) -ne 0)
...
function broadcastDesktopLyricsLockState() {
  ...
  if (mainWindow && !mainWindow.isDestroyed()) {
    mainWindow.webContents.send('susumusic-desktop-lyrics-lock-state', { locked });
  }
...
function broadcastDesktopLyricsEnabledState(enabled) {
  if (mainWindow && !mainWindow.isDestroyed()) {
    mainWindow.webContents.send('susumusic-desktop-lyrics-enabled-state', { enabled: !!enabled });
  }
}
...
function sendDesktopLyricsState() {
  if (!desktopLyricsWindow || desktopLyricsWindow.isDestroyed()) return;
  desktopLyricsWindow.webContents.send('susumusic-desktop-lyrics-state', desktopLyricsState);
}
...
function createDesktopLyricsWindow(payload = {}) {
  ...
  desktopLyricsWindow = new BrowserWindow({
    ...
    skipTaskbar: true,
    show: false,
    title: 'Susumusic Desktop Lyrics',
    webPreferences: {
...
if (-not ("SusumusicNativeWin" -as [type])) {
Add-Type @"
using System;
using System.Runtime.InteropServices;
public class SusumusicNativeWin {
...
$progman = [SusumusicNativeWin]::FindWindow("Progman", $null)
$result = [IntPtr]::Zero
[SusumusicNativeWin]::SendMessageTimeout($progman, 0x052C, [IntPtr]::Zero, [IntPtr]::Zero, 0, 1000, [ref]$result) | Out-Null
$script:workerw = [IntPtr]::Zero
$enum = [SusumusicNativeWin+EnumWindowsProc]{
  param([IntPtr]$top, [IntPtr]$param)
  $shell = [SusumusicNativeWin]::FindWindowEx($top, [IntPtr]::Zero, "SHELLDLL_DefView", $null)
  if ($shell -ne [IntPtr]::Zero) {
    $script:workerw = [SusumusicNativeWin]::FindWindowEx([IntPtr]::Zero, $top, "WorkerW", $null)
  }
  return $true
}
[SusumusicNativeWin]::EnumWindows($enum, [IntPtr]::Zero) | Out-Null
if ($script:workerw -eq [IntPtr]::Zero) { $script:workerw = $progman }
$target = [IntPtr]::new([Int64]${hwnd})
[SusumusicNativeWin]::SetParent($target, $script:workerw) | Out-Null
[SusumusicNativeWin]::SetWindowPos($target, [IntPtr]::Zero, 0, 0, 0, 0, 0x0013) | Out-Null
`;
...
function sendWallpaperState() {
  if (!wallpaperWindow || wallpaperWindow.isDestroyed()) return;
  wallpaperWindow.webContents.send('susumusic-wallpaper-state', wallpaperState);
}
...
ipcMain.handle('susumusic-hotkeys-configure-global', (_event, bindings) => {
  return configureSusumusicGlobalHotkeys(bindings);
});

ipcMain.handle('susumusic-export-json-file', async (event, payload = {}) => {
  try {
    const owner = getSenderWindow(event);
    const defaultName = String(payload.defaultName || 'susumusic-export.json').replace(/[\\/:*?"<>|]+/g, '-');
    const result = await dialog.showSaveDialog(owner, {
      title: '导出 Susumusic 存档',
...
ipcMain.handle('susumusic-import-json-file', async (event) => {
  try {
    const owner = getSenderWindow(event);
    const result = await dialog.showOpenDialog(owner, {
      title: '导入 Susumusic 存档',
      properties: ['openFile'],
      filters: [{ name: 'JSON', extensions: ['json'] }],
    });
...
ipcMain.handle('susumusic-open-update-installer', async (_event, filePath) => {
  try {
...
ipcMain.handle('susumusic-restart-app', async () => {
  try {
    app.relaunch();
    app.exit(0);
    return { ok: true };
...
ipcMain.handle('susumusic-desktop-lyrics-set-enabled', async (_event, enabled, payload) => {
  try {
    if (enabled) {
...
ipcMain.handle('susumusic-desktop-lyrics-update', async (_event, payload) => {
  try {
...
ipcMain.handle('susumusic-desktop-lyrics-set-dragging', async () => {
  return { ok: true };
});

ipcMain.handle('susumusic-desktop-lyrics-set-pointer-capture', async (_event, active) => {
  try {
...
ipcMain.handle('susumusic-desktop-lyrics-set-hot-bounds', async (_event, bounds) => {
  try {
...
ipcMain.handle('susumusic-desktop-lyrics-set-lock-state', async (_event, locked) => {
  try {
...
ipcMain.handle('susumusic-desktop-lyrics-move-by', async (_event, dx, dy) => {
  try {
...
ipcMain.handle('susumusic-wallpaper-set-enabled', async (_event, enabled, payload) => {
  try {
...
ipcMain.handle('susumusic-wallpaper-update', async (_event, payload) => {
  try {
...
async function createWindow() {
  ...
  process.env.SUSUMUSIC_UPDATE_DIR = getUpdateDownloadDir();
  try {
