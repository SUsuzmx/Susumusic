...
const UPDATE_WORK_DIR = process.env.SUSUMUSIC_UPDATE_DIR || path.join(__dirname, 'updates');
const UPDATE_DOWNLOAD_DIR = process.env.SUSUMUSIC_UPDATE_DOWNLOAD_DIR || path.join(UPDATE_WORK_DIR, 'downloads');
const UPDATE_PATCH_BACKUP_DIR = process.env.SUSUMUSIC_PATCH_BACKUP_DIR || path.join(UPDATE_WORK_DIR, 'backups', 'patches');
const BEATMAP_CACHE_DIR = process.env.SUSUMUSIC_BEAT_CACHE_DIR || (process.platform === 'win32' ? 'D:\\SusumusicCache\\beatmaps' : path.join(__dirname, 'beatmap-cache'));
const APP_PACKAGE = readPackageInfo();
const APP_VERSION = process.env.SUSUMUSIC_VERSION || APP_PACKAGE.version || '0.9.11';
...
function readUpdateConfig(pkg) {
  const local = (pkg && pkg.susumusic && pkg.susumusic.update) || {};
  const repoHint = process.env.SUSUMUSIC_UPDATE_REPOSITORY
  ...
  const owner = process.env.SUSUMUSIC_UPDATE_OWNER || local.owner || parsed.owner || '';
  const repo = process.env.SUSUMUSIC_UPDATE_REPO || local.repo || parsed.repo || '';
  return {
    ...
    manifest: process.env.SUSUMUSIC_UPDATE_MANIFEST
      || process.env.SUSUMUSIC_UPDATE_MANIFEST_URL
      || process.env.SUSUMUSIC_UPDATE_MANIFEST_FILE
      || '',
  };
}
...
function readUpdateMirrors(local) {
  const envMirrors = process.env.SUSUMUSIC_UPDATE_MIRRORS || process.env.SUSUMUSIC_UPDATE_MIRROR || '';
...
function normalizeManifestUpdateInfo(data) {
  ...
  const patchInfo = patch && patch.downloadUrl ? {
    name: patch.name || updateAssetNameFromUrl(patch.downloadUrl) || `Susumusic-${APP_VERSION}→${latestVersion}.patch.json`,
  ...
  const assetInfo = downloadUrl ? {
    name: asset.name || updateAssetNameFromUrl(downloadUrl) || `Susumusic-${latestVersion}-Setup.exe`,
  ...
  return {
    ...
    release: {
      ...
      name: release.name || data.name || ('Susumusic v' + latestVersion),
...
async function readUpdateManifest(ref) {
  ...
  if (/^https?:\/\//i.test(value)) {
    const resp = await fetch(value, {
      headers: { 'User-Agent': `Susumusic/${APP_VERSION}` },
    });
...
function beatCacheRootInfo() {
  ...
  // app 本地路径或非 Windows 平台直接放行；Windows 上仍禁止写入 C 盘
  const isAppLocal = dir.startsWith(path.resolve(__dirname));
  const isWindows = process.platform === 'win32';
  const allowed = isAppLocal || !isWindows || (!!root && !/^C:$/i.test(drive));
  const available = allowed && (!isWindows || fs.existsSync(root));
...
function parseLatestYmlUpdateInfo(text, reason) {
  ...
  const assetPath = yamlScalar(text, 'path') || yamlScalar(text, 'url') || `Susumusic-${latestVersion}-Setup.exe`;
  ...
  return {
    ...
    release: {
      ...
      name: 'Susumusic v' + latestVersion,
      version: latestVersion,
...
async function fetchLatestUpdateInfo() {
  ...
  try {
    const resp = await fetch(apiUrl, {
      ...
      headers: {
        'User-Agent': `Susumusic/${APP_VERSION}`,
    ...
    return {
      ...
      release: {
        ...
        name: data.name || ('Susumusic v' + latestVersion),
        version: latestVersion,
...
function safeUpdateFileName(name, version) {
  const raw = String(name || '').trim() || `Susumusic-${version || APP_VERSION}.exe`;
  const cleaned = raw
    .replace(/[<>:"/\\|?*\x00-\x1F]/g, '-')
    .replace(/\s+/g, ' ')
    .trim()
    .slice(0, 160);
  return cleaned || `Susumusic-${version || APP_VERSION}.exe`;
}
...
function writePatchFile(job, file) {
  ...
  const tmp = target + '.susumusic-patch';
...
function normalizePatchPayload(payload) {
  ...
  if (type && type !== 'susumusic-resource-patch') throw new Error('UNSUPPORTED_PATCH_TYPE');
...
async function handleDiscoverHome() {
  ...
  if (result[3].status === 'fulfilled' && result[3].value) {
    ...
    dailySongs = (Array.isArray(raw) ? raw : [])
      ...
      .slice(0, 20);
    // 补齐缺失封面：用 song_detail 批量查询
    const missingCovers = dailySongs.filter(s => !s.cover).map(s => s.id);
    if (missingCovers.length) {
      try {
        const dd = await song_detail({ ids: missingCovers.join(','), cookie: userCookie });
        const songsArr = (dd.body && dd.body.songs) || [];
        const idToPic = {};
        songsArr.forEach(s => {
          const pic = (s.al && s.al.picUrl) || (s.album && s.album.picUrl) || '';
        ...
        dailySongs = dailySongs.map(s => s.cover ? s : { ...s, cover: idToPic[s.id] || '' });
      } catch (e) { console.warn('[DiscoverHome] cover backfill failed:', e.message); }
    }
  }
...
const server = http.createServer(async (req, res) => {
  ...
  if (pn === '/api/app/version') {
    sendJSON(res, {
      name: APP_PACKAGE.name || 'susumusic',
      productName: APP_PACKAGE.productName || 'Susumusic',
      version: APP_VERSION,
      update: {
  ...
  // ---------- 相似歌曲 ----------
  if (pn === '/api/song/similar') {
    try {
      const id = url.searchParams.get('id');
      if (!id) { sendJSON(res, { error: 'Missing song id', songs: [] }, 400); return; }
      const limit = Math.max(5, Math.min(30, parseInt(url.searchParams.get('limit') || '20', 10) || 20));
      const r = await simi_song({ id, cookie: userCookie, timestamp: Date.now() });
      const b = r.body || r || {};
      const rawSongs = b.songs || [];
      const songs = rawSongs.map(mapSongRecord).filter(s => s.id).slice(0, limit);
      sendJSON(res, { songs });
    } catch (err) {
      console.warn('[SimilarSongs]', err.message);
