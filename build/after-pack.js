...
function resolveRcedit(projectDir) {
  ...
  if (!hit) throw new Error('No usable rcedit executable was found for Susumusic icon injection.');
  return hit;
}
...
module.exports = async function afterPack(context) {
  ...
  const appName = context.packager.appInfo.productFilename || 'Susumusic';
  ...
  if (!fs.existsSync(exePath)) throw new Error(`Susumusic executable was not found: ${exePath}`);
  if (!fs.existsSync(iconPath)) throw new Error(`Susumusic icon was not found: ${iconPath}`);

  const version = context.packager.appInfo.version;
  console.log(`  • injecting Susumusic resources  rcedit=${rceditPath}`);
  execFileSync(rceditPath, [
    exePath,
    '--set-icon', iconPath,
    '--set-version-string', 'FileDescription', 'Susumusic',
    '--set-version-string', 'ProductName', 'Susumusic',
    '--set-version-string', 'CompanyName', 'Susumusic',
